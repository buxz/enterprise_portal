<%@ page import="com.buxz.enterprise_portal.entity.*" %>
        ...
        if (methon.equals("POST")) {
            try {
                ...
                if (login.LoginCheck(User, Pwd)) {
                    ...
                    out.println("<SCRIPT LANGUAGE='JavaScript'>alert('登录成功！');location.href='/admin/news';</SCRIPT>");
                    return;
                } else {
                    out.println("<SCRIPT LANGUAGE='JavaScript'>alert('用户名或密码不正确！');location.href='/admin';</SCRIPT>");

                    return;
                }
            } catch (Exception e) {
                out.println("<SCRIPT LANGUAGE='JavaScript'>alert('服务器异常！!');location.href='/admin';</SCRIPT>");
                return;
            }
        } else {
...
<div class="am-g">
    <div class="am-u-lg-6 am-u-md-8 am-u-sm-centered">
        <form action="<%=basePath%>/admin/login" method="post" class="am-form login-form" onSubmit="return LoginCheck()">
            <label for="User">
                用户名:
            </label>
            <!-- 文本输入框  用于输入用户名 -->
